...
const QuizResultDetail = ({ show, onHide, result, assignment }) => {
    ...
    const studentName = result.studentId?.profile?.name || result.studentId?.profile?.fullName || result.studentName || 'Unknown Student';
    const studentEmail = result.studentId?.email || result.studentEmail || 'No email';
    const actualScore = result.grade?.score || result.score || 0;
    const completedAt = result.submittedAt || result.completedAt;
    const timeSpent = result.timeSpent || 0;

    // Use scale of 10 for display
    const maxScore = 10;
    const percentage = maxScore > 0 ? Math.round((actualScore / maxScore) * 100) : 0;

    // Process answers with correct validation
    const processedAnswers = result.answers?.map(answer => {
        const question = assignment?.questions?.find(q => q.questionId === answer.questionId);
    ...
    // Use questions from assignment, fallback to mock if needed
    const mockQuestions = [
        {
            questionId: "q1",
                            ...
                            <Col md={6}>
                                ...
                                <div className="d-grid gap-2">
                                    <div className="d-flex justify-content-between">
                                        ...
                                        <span className="fw-medium">{completedAt ? formatDateTime(completedAt) : "Not specified"}</span>
                                    </div>
                                    <div className="d-flex justify-content-between">
                                        <span className="text-muted">Time Spent:</span>
                                        <span className="fw-medium">{timeSpent} minutes</span>
                                    </div>
                                </div>
                            </Col>
                            <Col md={6}>
                                <Row className="g-3">
                                    ...
                                    <Col md={6}>
                                        <Card className="text-center bg-light">
                                            <Card.Body className="py-3">
                                                <div className={`h3 fw-bold text-${getGradeColor(percentage)} mb-1`}>
                                                    {percentage}%
                                                </div>
                                    ...
                                    <Col md={12}>
                                        <Card className="text-center">
                                            <Card.Body className="py-3">
                                                <Badge bg={getGradeColor(percentage)} className="fs-5 px-3 py-2">
                ...
                <Card className="mb-4">
                    ...
                    <Card.Body>
                        <Row className="text-center">
                            <Col md={3}>
                                <div className="p-3">
                                    <div className="h4 fw-bold text-success mb-1">
                                        {processedAnswers.filter(a => a.isCorrect).length}
                                    </div>
                            ...
                            <Col md={3}>
                                <div className="p-3">
                                    <div className="h4 fw-bold text-primary mb-1">{processedAnswers.length}</div>
                            ...
                            <Col md={3}>
                                <div className="p-3">
                                    <div className="h4 fw-bold text-info mb-1">
                                        {processedAnswers.reduce((sum, a) => sum + (a.points || 0), 0)}
                                    </div>
                    ...
                    <Card.Body>
                        <div className="d-grid gap-4">
                            {questions.map((question, index) => {
                                const studentAnswer = processedAnswers.find(a => a.questionId === question.questionId)
            ...
            <Modal.Footer>
                <div className="d-flex justify-content-between w-100 align-items-center">
                    <div className="text-muted small">
                        Quiz: {assignment?.title} • Completed at: {completedAt ? formatDateTime(completedAt) : "Not specified"}
                    </div>
